...
void Init_SD()
{
	...
	if (ret != ESP_OK) {
        lprintf(LO_INFO, "Failed to initialize SPI bus. :(\n");
        return;
    }
...
int I_Open(const char *wad, int flags) {
	...
	if(init_SD == false) {
		lprintf(LO_INFO, "I_Open: SD not initialized, initializing...\n");
		Init_SD();
	}
	
	int x=0;
	for (x = 0; x < 32; x++) {
		lprintf(LO_INFO, "I_Open: Checking slot %d: file=%p, name=%s\n", x, fds[x].file, fds[x].name);
		if (fds[x].file == NULL || strcmp(fds[x].name, fname) == 0) {
			break;
		}
	}
	...
	if (strcmp(fname, fileName)==0) {
		...
		lprintf(LO_INFO, "I_Open: Opening file %s on /sdcard\n", fname);
